...
Reference implementation: Google (covers Gmail, Calendar, Drive). The
shape (`build_auth_url`, `exchange_code`) is provider-agnostic so adding
HubSpot, Microsoft, Salesforce is a copy-paste of the URLs + scope list.

State is signed with the configured `oauth_state_secret` so we can verify
the callback parameter wasn't tampered with. The encoded state carries
``tenant_id`` and ``rep_id`` so the callback handler can route the token
back to the right rep without any server-side session store.
"""
...
class ProviderConfig:
    ...
    def client_id(self, settings: Settings) -> str:
        return _attr_for(settings, f"oauth_{self.name}_client_id")

    def client_secret(self, settings: Settings) -> str:
        return _attr_for(settings, f"oauth_{self.name}_client_secret")

    def redirect_uri(self, settings: Settings) -> str:
        return _attr_for(settings, f"oauth_{self.name}_redirect_uri")


def _attr_for(settings: Settings, name: str) -> str:
...
async def exchange_code(
    *,
    ...
    """Returns the decoded state + a normalised OAuthToken-shaped dict."""
    settings = settings or get_settings()
    config = _get_provider(provider)
